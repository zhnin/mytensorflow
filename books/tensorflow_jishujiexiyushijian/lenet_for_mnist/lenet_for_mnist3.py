# @Time    : 2018/7/5 11:25
# @Author  : cap
# @FileName: lenet_for_mnist3.py
# @Software: PyCharm Community Edition
# @introduction:
# 三次优化
# 添加learning-rate优化，添加regulation优化，添加滑动评价优化
# 一二卷积输出数据标准化，三四层添加dropout
# 第一层：conv1--relu1-pool1
# 第二层；conv2--reul2-pool2
# 第三层：fc1-relu
# 第四层(输出层)：fc2
# 运行10000次后，validation accuracy：0.98280；test accuracy:0.97900
import numpy as np
...
# 定义计算图
def inference(x, dropout, regularizer=None):
    ...
    with tf.variable_scope('layer3'):
        ...
        fc1_out = tf.nn.dropout(fc1_out, dropout)
...
def train(mnist):
    ...
    logit = inference(x, drop_out, regularizer)
    ...
    with tf.Session() as sess:
        ...
        for i in range(steps):
            ...
            _, step = sess.run([train_op, global_step], feed_dict={x: batch_x, y: batch_y, drop_out:0.5})
            if step % 1000 == 0:
                validation_accu = sess.run(accuracy, feed_dict={x: validation_x, y:validation_y, drop_out: 1.0})
                print('After {} steps, accuracy:{:.5f}'.format(step, validation_accu))

        print('train finished!')
        test_accu = sess.run(accuracy, feed_dict={x: test_x, y: test_y, drop_out: 1.0})
